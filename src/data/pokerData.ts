...
export interface CourseChapter {
  ...
  title: string;
  readTime: string;
  category: 'Math' | 'Preflop' | 'Postflop' | 'ICM' | 'Live & Mental';
  summary: string;
  keyTakeaways: string[];
  content: string;
}
...
- **同花聽牌 (Flush Draw)**：9 張出路 $\\rightarrow 9 \\times 4 = 36\\%$ (翻牌到河牌) / $9 \\times 2 = 18\\%$ (轉牌到河牌)。
- **兩頭順子聽牌 (OESD)**：8 張出路 $\\rightarrow 8 \\times 4 = 32\\%$ (翻牌到河牌) / $8 \\times 2 = 16\\%$ (轉牌到河牌)。
- **卡順 (Gutshot)**：4 張出路 $\\rightarrow 4 \\times 4 = 16\\%$。
